...
class DCT
{
	...
	for (int i = 0; i < 8; i++) {
		...
		DivisorsLuminance[index]
		    = 1.0 / ((double) quantum_luminance[index]
	...
	for (int i = 0; i < 8; i++) {
		...
		DivisorsChrominance[index]
		    = 1.0 / ((double) quantum_chrominance[index]
			     * AANscaleFactor[i] * AANscaleFactor[j] * 8.0);
		index++;
	    }
	}
	...
	for (int v = 0; v < 8; v++) {
		...
		double[] ds = output[v];
		int i = u;
		ds[i] = ds[i] * (0.25 * (u == 0 ? 1.0 / Math.sqrt(2.0) : 1.0)
