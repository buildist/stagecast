...
class Huffman
{
	...
	= { 16, 0, 2, 1, 3, 3, 2, 4, 3, 5, 5, 4, 4, 0, 0, 1, 125 };
	...
	= { 17, 0, 2, 1, 2, 4, 4, 3, 4, 7, 5, 4, 4, 0, 1, 2, 119 };
    ...
    public void HuffmanBlockEncoder(BufferedOutputStream outStream,
				    int[] zigzag, int prec, int DCcode,
				    int ACcode) {
	NumOfDCTables = 2;
	NumOfACTables = 2;
	int temp2;
	...
	for (int k = 1; k < 64; k++) {
		...
		for (/**/; r > 15; r -= 16)
		    bufferIt(outStream, ((int[][]) AC_matrix[ACcode])[240][0],
			     ((int[][]) AC_matrix[ACcode])[240][1]);
		temp2 = temp;
		if (temp < 0) {
		    temp = -temp;
		    temp2--;
